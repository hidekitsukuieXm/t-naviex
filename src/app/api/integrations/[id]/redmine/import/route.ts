/**
 * Redmine Import API
 * POST /api/integrations/[id]/redmine/import - Import issue from Redmine
 */
...
import { importIssueFromRedmine } from '@/services/redmine-sync';
...
// POST /api/integrations/[id]/redmine/import
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const { externalIssueId, projectId } = validation.data;

    const result = await importIssueFromRedmine(
      integrationId,
      externalIssueId,
      projectId,
    ...
    return NextResponse.json({ error: 'Redmineからのインポートに失敗しました。' }, { status: 500 });
  }
}
