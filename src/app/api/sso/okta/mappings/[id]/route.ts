...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (mapping.configuration.providerName !== 'OKTA') {
      return NextResponse.json({ error: 'Oktaのマッピングではありません' }, { status: 400 });
    }
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (existingMapping.configuration.providerName !== 'OKTA') {
      return NextResponse.json({ error: 'Oktaのマッピングではありません' }, { status: 400 });
    }
...
/**
 * DELETE /api/sso/okta/mappings/[id]
 * ロールマッピングを削除
 */
