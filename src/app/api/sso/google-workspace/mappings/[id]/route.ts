/**
 * Google Workspace Role Mapping Detail API
 *
 * 個別のGoogle Workspaceロールマッピング管理
 */
...
/**
 * GET /api/sso/google-workspace/mappings/[id]
 * 特定のロールマッピングを取得
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (mapping.configuration.providerName !== 'GOOGLE') {
      return NextResponse.json(
        { error: 'Google Workspaceのマッピングではありません' },
        { status: 400 }
      );
    }
...
/**
 * PUT /api/sso/google-workspace/mappings/[id]
 * ロールマッピングを更新
 */
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (existingMapping.configuration.providerName !== 'GOOGLE') {
      return NextResponse.json(
        { error: 'Google Workspaceのマッピングではありません' },
        { status: 400 }
      );
...
/**
 * DELETE /api/sso/google-workspace/mappings/[id]
 * ロールマッピングを削除
 */
