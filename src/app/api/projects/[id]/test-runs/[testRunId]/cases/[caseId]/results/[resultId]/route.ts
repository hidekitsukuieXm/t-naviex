 ...
 * PUT /api/projects/[id]/test-runs/[testRunId]/cases/[caseId]/results/[resultId] - 結果更新
...
import type { UpdateTestResultInput } from '@/types/test-result';
...
// PUT /api/projects/[id]/test-runs/[testRunId]/cases/[caseId]/results/[resultId]
export async function PUT(request: Request, { params }: RouteParams) {
  try {
    ...
    const body = (await request.json()) as UpdateTestResultInput;

    const result = await updateTestResult(BigInt(resultId), body, BigInt(session.user.id));
    ...
    return NextResponse.json({ error: '結果の更新に失敗しました。' }, { status: 500 });
  }
}
